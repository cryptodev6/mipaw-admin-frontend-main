...
export class ProductComponent implements AfterViewInit, OnInit {
  @ViewChild("productsSearch", { static: true })
  productsSearchInput: ElementRef;
  ...
  inflateData() {
    let that = this;
    this._productService.get(this.filters).subscribe((resp: any) => {
      
      //In case there are no products in database for this store
      if (resp.count == 0) this.noProducts = true;
      else this.noProducts = false;
      
      this.preloader = false;
      const productsList = (resp.data || []).flatMap((obj) => obj.products);
      ...
      //Finding total pages
      let totalPages = Math.ceil(resp.count / that.filters.limit);

      let pStr = "";
      for (let i = 1; i <= totalPages; i++) pStr += i + " ";
  ...
  initSearchFunctionality(): void {
    ...
    fromEvent($("#inputSearchProducts"), "keyup")
      .pipe(
        //get value
        map((event: any) => {
  ...
  deleteProducts() {
    ...
    if (productsLst.length > 0) {
      ...
      this._productService.remove({ ids: ids }).subscribe(
        (resp) => {
